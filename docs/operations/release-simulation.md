...
The validation checks image user/entrypoint/health metadata and OCI labels, asserts that no secret-bearing environment is baked into any image, and runs all three images under the same read-only/non-root/capability restrictions. It also checks the exact Node/OpenSSL versions, rejects a QUIC runtime and shell/package-manager binaries, and exercises the assembled Node entrypoint. API/worker inspection rejects source, test, map, TypeScript build, Prisma CLI, and development-dependency artifacts; migrator inspection requires only its locked Prisma migration runtime, schema, committed migrations, and verifier.
