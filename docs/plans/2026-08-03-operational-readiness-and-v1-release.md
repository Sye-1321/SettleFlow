...
- create historical synthetic Payment/capture evidence only through existing Payment/Idempotency/Ledger/Eventing services with an injected fixed clock/ID source, so every financial invariant and outbox rule remains identical to normal behavior;
...
| `Dockerfile`, `.dockerignore`, `compose.release.yaml`, `compose.yaml` only if shared health metadata is needed                                                                | API/worker OCI targets, one-shot jobs, isolated release-simulation topology, health/resources/telemetry profiles               | ADR-0001/0005 deployable model preserved                                                  |
...
- [ ] Approved coverage gates remain blocking; focused test remediation must not change their thresholds or exclusions.
- [ ] Runtime image vulnerability gates remain blocking; the pinned Debian base has unreviewed high and critical findings and no exception was created.
- [ ] Performance and recovery gates remain deferred to their approved Step 7-9 prerequisites; they are not represented as passing CI checks.
...
| Approved coverage floors            | Blocked | 2026-08-10: all 226 instrumented tests passed; aggregate statements/branches/functions/lines were 62.45/53.93/50.45/63.58%, below the approved 85/80/80/85% floors, and every critical module remained below at least one 90/85/85/90 floor; thresholds and exclusions were not changed                                                                                                                                                                                                                      |
