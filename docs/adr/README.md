...
Accepted ADR-0006 records the separate payment/settlement lifecycle baseline and physical ownership refinement. ADRs 0006 through 0013 now provide the accepted pre-implementation Payment Intent decisions; implementation-specific contract artifacts and the settlement read-composition design remain required at their recorded gates. Standalone repository ADRs for specification ADR-006 and ADR-007 are **To be created** when their affected implementation milestones require them. The indexed ADRs do not authorize Redis, combined lifecycles, or weakened webhook SSRF controls.
