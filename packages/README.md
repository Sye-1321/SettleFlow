...
`modules/webhooks` owns merchant-scoped endpoint, subscription, encrypted-secret, URL-policy, lifecycle application behavior, retained processed-event markers, eligibility selection, and pending delivery projection. It depends on the Operations audit port for endpoint lifecycle evidence and on Eventing's public inbox/message types for the projection effect. Eventing consumes RabbitMQ; Webhooks never acknowledges broker messages and does not send HTTP requests or own payment state.
