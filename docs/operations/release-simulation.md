...
The multi-stage root `Dockerfile` uses the exact Node.js 24.18.0 Bookworm-slim image digest, pnpm 11.18.0, and exact OpenSSL runtime packages. The API and worker final stages contain production dependencies and compiled JavaScript only; npm, Corepack, Yarn, and their command links are removed from every runtime target after the build because normal operation invokes Node directly. They run as fixed UID/GID `10001:10001`, use exec-form Node entrypoints, accept `SIGTERM`, have read-only root filesystems and bounded `/tmp` filesystems, drop all Linux capabilities, and prohibit privilege escalation. OCI source, revision, version, and created labels are set from the generated configuration; BuildKit generates SBOM and provenance attestations while building.
