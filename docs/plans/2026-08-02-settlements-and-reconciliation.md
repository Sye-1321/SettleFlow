...
The project owner approved Gates 1 through 10 exactly as recorded below on 2026-08-03. ADR-0021 was subsequently Accepted and committed in the clean `852240a` baseline, resolving the Ledger chart and guarded-posting prerequisite before implementation began.
...
The implementation baseline is clean commit `852240a` on `main`, aligned with `origin/main`. The earlier planning inspection began at `556de6a` after the committed Payment Capture and Refund milestone.
...
1. Apply accepted ADR-0021. Gates 1 through 10 fix the execution, fee, cutoff, adjustment, reconciliation, actor, event, and Webhook contracts for this plan.
...
## Affected modules and implementation files

The implementation changed the following planned areas; the verification record and outcomes below describe the final boundary refinements.
...
| `prisma/migrations/<timestamp>_settlement_ledger_reference_snapshot/migration.sql`                                                                                                                                                                     | Add immutable public Ledger linkage without a cross-domain read                     | Additive backfill and guarded evidence     |
...
| `packages/modules/payments/src/{payments.types,prisma-payment-settlement.reader,index}.ts` and tests                                                                                                                                                   | Expose transaction-aware Settlement revalidation/projection identity port           | Payments-owned reads and locks only        |
...
The following repository commands are the completed implementation gates. Migration commands use the owner-only local `MIGRATION_DATABASE_URL`; application and worker tests use the non-owner runtime role where applicable.
...
Focused gates are `pnpm test:settlements`, `pnpm test:reconciliation`, and `pnpm test:event-contract`. The real PostgreSQL Settlement/Reconciliation integration test covers competing runs, post-settlement refund adjustment, tenant isolation, immutable evidence, runtime permissions, and the 500-item continuation boundary. The CSV unit suite covers the exact 50,000-row boundary. JSON parsing and repository-local Markdown-link resolution are included in the final documentation checks recorded below.
...
| ---------------------------------------------------------- | ----------------------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------- |
| ADR-0020 closed chart/type excluded Settlement             | No valid fee/clearing posting                   | Resolved by accepted ADR-0021 and invariant/permission integration tests | Resolved before migration                         |
...
| Streaming CSV input is untrusted                           | Resource, parser, or supply-chain vulnerability | Exact `csv-parse@7.0.1`, Gate 9 limits, strict parsing, and abuse tests  | Controlled in this implementation                 |
...
1. Accept ADR-0021; Gates 1 through 10 and their exact decisions are approved and recorded here.
...
- [x] ADR-0021 reviewed and Accepted before Ledger/schema/application implementation.
- [x] Ledger extension, Settlement/Reconciliation schema, and migrations implemented/verified.
- [x] API/worker/event/Webhook implementations completed in approved scope.
- [x] Financial, concurrency, failure, security, permission, bounded-workload, and regression tests pass.
- [x] Documentation, examples, OpenAPI, event schemas, and runbooks updated.
...
| ------------------------------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Baseline status/HEAD                              | Pass   | 2026-08-03: clean `## main...origin/main` at `852240a`; no implementation changes existed before this milestone                                                                                    |
| Authoritative specification/repository inspection | Pass   | Complete v1.0 specification plus governance, architecture, accepted ADR-0020/0021, schema/migrations, modules, tests, plans, docs, and runbooks inspected                                          |
| Specification authorization                       | Pass   | FR-11/FR-12 authorize the slice; Gates 1-10 and ADR-0021 were Accepted before implementation                                                                                                       |
| Dependency installation                           | Pass   | `corepack pnpm@11.18.0 install --frozen-lockfile`; all 13 workspace projects current; exact `csv-parse@7.0.1` installed                                                                            |
| Local infrastructure                              | Pass   | `docker compose ps`: PostgreSQL 18.4 and RabbitMQ 4.3.4 healthy                                                                                                                                    |
| Prisma and migrations                             | Pass   | Prisma 7.9.1 validation/generation/status passed; all 11 migrations applied from empty in disposable PostgreSQL and upgraded the prior local database; datasource-to-schema diff returned empty    |
| Formatting, lint, and type-check                  | Pass   | `pnpm format:check`, `pnpm lint`, and `pnpm typecheck` passed                                                                                                                                      |
| Unit and contract tests                           | Pass   | 44 suites / 196 tests passed across API, worker, and nine domain projects; exact 50,000-row CSV boundary and operational Webhook projection contracts passed                                       |
| Integration tests                                 | Pass   | 10 suites / 61 tests passed against real PostgreSQL/RabbitMQ; focused Settlement/Reconciliation rerun also passed with competing-run, adjustment, tenant, permissions, and 500+1 capacity evidence |
| Production build                                  | Pass   | Root API and worker production build passed                                                                                                                                                        |
| OpenAPI and event artifacts                       | Pass   | Generated OpenAPI drift check passed; exact Settlement/Reconciliation event JSON schemas parsed successfully                                                                                       |
| Documentation and repository hygiene              | Pass   | README/API/architecture/event/runbook/example updates inspected; Markdown links, formatting, JSON parsing, `git diff --check`, and final Git status recorded in the implementation report          |

### Implementation outcomes and deviations

- Settlement execution remains the approved synchronous, merchant-scoped simulation. `SETTLED` means finalized into `settlement_clearing`; no payout or external rail call occurs.
- Reconciliation staging uses exact `csv-parse@7.0.1`; processing runs in the worker through explicit Payments, Settlements, and Ledger read ports rather than cross-domain table access in Reconciliation.
- Payments owns the reviewed transaction-aware batch-revalidation and projection-identity adapter. Ledger supplies an opaque internal linkage plus its public ID; Settlement snapshots the public ID so its reads never query Ledger persistence.
- Settlement lifecycle projection and Webhook projection have separate durable consumers/connections. Outbox publishing remains at least once, and exact event bytes are retained for signed Webhook delivery.
- The migrations wrap each milestone atomically, backfill accounts/positions/projection discriminators, add guarded triggers and named constraints, and grant the non-owner runtime role only the required operations.
- The bounded workload checks prove one 501-item candidate set becomes immutable batches of 500 and 1, and that exactly 50,000 CSV rows are accepted while row 50,001 is rejected. No production-scale multi-merchant benchmark or alert-threshold tuning is claimed.
- Operations alert destinations/materiality thresholds and destructive retention remain deferred; structured signals, safe backlog queries, and recovery runbooks are included now.

## Definition of done

Planning approval was recorded before implementation, and accepted ADR-0021 resolved the sole architecture blocker.
